...
Type *Type::k_types[] = {
    new Type(TypeId::TYPE_EMPTY),
    // new BooleanType(),
    // new TinyintType(),
    // new SmallintType(),
    new IntegerType(TypeId::TYPE_INT), new BigintType(), new DecimalType(), new DecimalType(),
    new VarlenType(TypeId::TYPE_VARCHAR), new VarlenType(TypeId::TYPE_VARCHAR), new TimestampType(),
    //  new VectorType()
};
...
auto Type::IsCoercableFrom(const TypeId type_id) const -> bool {
  switch (type_id_) {
    ...
    case TYPE_DOUBLE:
      ...
      break;
    case TYPE_DATE:
      return (type_id == TYPE_VARCHAR || type_id == TYPE_DATE);
    case TYPE_VARCHAR:
      switch (type_id) {
        // case BOOLEAN:
        // case TINYINT:
...
auto Type::GetMinValue(TypeId type_id) -> Value {
  switch (type_id) {
    ...
    case TYPE_LONG:
      return {type_id, EASYDB_INT64_MIN};
    case TYPE_FLOAT:
    case TYPE_DOUBLE:
      return {type_id, EASYDB_DECIMAL_MIN};
    case TYPE_DATE:
      return {type_id, 0};
    case TYPE_VARCHAR:
      return {type_id, ""};
...
auto Type::GetMaxValue(TypeId type_id) -> Value {
  switch (type_id) {
    ...
    case TYPE_LONG:
      return {type_id, EASYDB_INT64_MAX};
    case TYPE_FLOAT:
    case TYPE_DOUBLE:
      return {type_id, EASYDB_DECIMAL_MAX};
    case TYPE_DATE:
      return {type_id, EASYDB_DATE_MAX};
    case TYPE_VARCHAR:
