...
struct ColMeta {
  ...
  ColMeta(std::string tab_name_, std::string name_, ColType type_, int len_,
          int offset_, bool index_)
      : tab_name(tab_name_),
        name(name_),
        type(type_),
        len(len_),
...
struct TabMeta {
  ...
  TabMeta(std::string name_, std::vector<ColMeta> cols_,
          std::vector<IndexMeta> indexes_, Schema schema_)
  ...
  uint32_t GetColId(const std::string &col_name) {
    return get_col(col_name) - cols.begin();
  }
