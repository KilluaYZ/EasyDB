...
class ProjectionPlan : public Plan {
 public:
  ...
  ProjectionPlan(PlanTag tag, std::shared_ptr<Plan> subplan,
                 std::vector<TabCol> sel_cols, bool is_unique = false) {
    Plan::tag = tag;
...
class DMLPlan : public Plan {
 public:
  ...
  DMLPlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::string tab_name,
          ...
          std::vector<SetClause> set_clauses, bool unique = false) {
    Plan::tag = tag;
    subplan_ = std::move(subplan);
    tab_name_ = std::move(tab_name);
...
class DDLPlan : public Plan {
 public:
   ...
   * @brief 构造函数
   * @param tag
   * 计划标签（T_CreateTable、T_DropTable、T_CreateIndex、T_DropIndex）
   * @param tab_name 表名
