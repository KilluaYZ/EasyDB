...
class Schema {
 public:
   ...
   * @param col_name 要查找的列名
   * @return
   * 如果找到，返回列的索引（包装在optional中）；如果不存在，返回std::nullopt
   * @note
   ...
   */
  void Append(std::vector<Column> app) {
    columns_.insert(columns_.end(), app.begin(), app.end());
  }
   ...
   * @return 元组的物理大小（字节数）
   * @note
   * 物理大小包括所有内联列的大小、指向非内联列的指针大小，以及非内联列的实际数据大小
   */
...
// template <typename T>
// struct fmt::formatter<T, std::enable_if_t<std::is_base_of<easydb::Schema,
// T>::value, char>>
...
// std::enable_if_t<std::is_base_of<easydb::Schema, T>::value, char>>
//     : fmt::formatter<std::string> {
//   template <typename FormatCtx>
