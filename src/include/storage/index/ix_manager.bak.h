...
class IxManager {
 ...
 public:
  ...
  void create_extendible_hash_index(const std::string &filename,
                                    const std::vector<ColMeta> &index_cols) {
    std::string ix_name = get_index_name(filename, index_cols);
    // Create index file
    disk_manager_->CreateFile(ix_name);
    ...
    // Create directory bucket page and write to file
    {
      ...
      *phdr = {.next_free_page_no = IX_NO_PAGE,
      ...
      tp_rids[0] = {.page_no = IX_INIT_BUCKET_0_PAGE,
                    .slot_no = IX_NO_PAGE};  // only page_no is valid
      tp_rids[1] = {.page_no = IX_INIT_BUCKET_1_PAGE,
                    .slot_no = IX_NO_PAGE};  // only page_no is valid
      // Must write PAGE_SIZE here in case of future fetch_node()
  ...
  IxExtendibleHashIndexHandle *open_hash_index(
      const std::string &filename, const std::vector<ColMeta> &index_cols) {
    std::string ix_name = get_index_name(filename, index_cols);
    int fd = disk_manager_->OpenFile(ix_name);
