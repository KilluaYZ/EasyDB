...
class Optimizer {
 ...
 public:
  ...
  std::shared_ptr<Plan> plan_query(std::shared_ptr<Query> query,
    ...
    } else if (auto x = std::dynamic_pointer_cast<ast::CreateStaticCheckpoint>(
                   query->parse)) {
      // create static_checkpoint;
      return std::make_shared<OtherPlan>(T_CreateStaticCheckpoint,
                                         std::string());
    } else if (auto x =
                   std::dynamic_pointer_cast<ast::LoadData>(query->parse)) {
      // load file_name into table_name;
      return std::make_shared<LoadDataPlan>(T_LoadData, x->file_name,
                                            x->tab_name);
    } else {
  ...
  bool bypass(std::shared_ptr<Query> query, Context *context) {
    if (query->tables.size() == 1 && query->cols.size() == 1 &&
        query->conds.empty()) {
