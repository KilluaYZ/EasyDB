...
static constexpr uint64_t TABLE_PAGE_HEADER_SIZE =
    Page::SIZE_PAGE_HEADER + sizeof(RmPageHdr);
...
class RmPageHandle {
 ...
 public:
  ...
  RmPageHandle(const RmFileHdr *fhdr_, Page *page_)
    ...
    page_hdr_ =
        reinterpret_cast<RmPageHdr *>(page->GetData() + page->OFFSET_PAGE_HDR);
    tuple_info_ = reinterpret_cast<TupleInfo *>(
        page->GetData() + sizeof(RmPageHdr) + page->OFFSET_PAGE_HDR);
  ...
  void SetNextPageId(page_id_t next_page_id) {
    page_hdr_->next_page_id = next_page_id;
  }
 ...
 private:
   ...
   * @brief 页面头指针
   * @note
   * page->data的第一部分，存储页面元信息，指针指向首地址，长度为sizeof(RmPageHdr)
   */
  RmPageHdr *page_hdr_;

  /**
   * @brief 元组信息数组指针
   * @note page->data的第二部分，存储页面的元组信息，长度为num_records *
   * sizeof(TupleInfo)
   */
  TupleInfo *tuple_info_;

  // char *bitmap;  //
  // page->data的第二部分，存储页面的bitmap，指针指向首地址，长度为file_hdr->bitmap_size
  // char *slots;  //
  // page->data的第三部分，存储表的记录，指针指向首地址，每个slot的长度为file_hdr->record_size
...
class RmFileHandle {
 ...
 public:
  ...
  RmFileHandle(DiskManager *disk_manager,
    ...
    // init file_hdr_
    disk_manager_->ReadPage(fd, RM_FILE_HDR_PAGE, (char *)&file_hdr_,
                            sizeof(file_hdr_));
   ...
   */
  auto InsertTuple(const TupleMeta &meta, const Tuple &tuple, Context *context)
      -> std::optional<RID>;
   ...
   */
  auto InsertTuple(RID rid, const TupleMeta &meta, const Tuple &tuple,
   ...
   * @return 元组的智能指针
   */
  auto GetTupleValue(const RID &rid, Context *context)
  ...
  //   return Bitmap::is_set(page_handle.bitmap, rid.GetSlotNum());  //
  //   page的slot_no位置上是否有record
  // }
   ...
   */
  auto GetKeyTuple(const Schema &schema, const Schema &key_schema,
                   const std::vector<uint32_t> &key_attrs, const RID &rid,
