...
enum SetKnobType {
    EnableNestLoop, EnableSortMerge, EnableOutput
};
...
struct Col : public Expr {
    ...
    Col(std::string tab_name_, std::string col_name_, std::string new_col_name_, AggregationType aggregation_type_) :
            tab_name(std::move(tab_name_)), col_name(std::move(col_name_)), new_col_name(std::move(new_col_name_))
...
struct SelectStmt : public TreeNode {
    ...
    SelectStmt(std::vector<std::shared_ptr<Col>> cols_,
            ...
            order(std::move(order_)), group(std::move(group_)), having(std::move(having_)) {
                has_sort = (bool)order;
