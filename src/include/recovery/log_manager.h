...
static std::string LogTypeStr[] = {"UPDATE", "INSERT", "DELETE",    "BEGIN",
...
class UpdateLogRecord : public LogRecord {
 public:
   ...
   * @param dest 目标缓冲区指针
   * @note 序列化格式：日志头部 + 旧值大小 + 旧值数据 + 新值大小 + 新值数据 +
   * RID + 表名大小 + 表名
   */
  void serialize(char *dest) const override {
    LogRecord::serialize(dest);
...
class CheckpointLogRecord : public LogRecord {
 public:
  ...
  CheckpointLogRecord(txn_id_t txn_id, lsn_t prev_lsn) : CheckpointLogRecord() {
    ...
    tab_name_str_ = "";
    log_tot_len_ += sizeof(lsn_t) + sizeof(size_t) * 5 +
                    tab_name_offset_size_ * sizeof(size_t);
  }

  /**
   * @brief 序列化checkpoint日志字段到dest中
   * @param dest 目标缓冲区指针
   * @note 序列化格式：日志头部 + ATT + 已中止事务列表 + DPT + 最小恢复LSN +
   * 表名信息
   */
  void serialize(char *dest) const override {
    LogRecord::serialize(dest);
    ...
    offset += sizeof(size_t);
    memcpy(dest + offset, att_vec_.data(),
           att_size_ * sizeof(std::pair<txn_id_t, lsn_t>));
    ...
    offset += sizeof(size_t);
    memcpy(dest + offset, aborted_txns_vec_.data(),
           aborted_txns_size_ * sizeof(txn_id_t));
    offset += aborted_txns_size_ * sizeof(txn_id_t);
    // dpt
    // size_t dpt_size = dpt_vec_.size();
    memcpy(dest + offset, &dpt_size_, sizeof(size_t));
    offset += sizeof(size_t);
    memcpy(dest + offset, dpt_vec_.data(),
           dpt_size_ * sizeof(std::pair<page_id_t, lsn_t>));
    ...
    offset += sizeof(size_t);
    memcpy(dest + offset, tab_name_offset_vec_.data(),
           tab_name_offset_size_ * sizeof(size_t));
  ...
  void deserialize(const char *src) override {
    ...
    memcpy(att_vec_.data(), src + offset,
           att_size_ * sizeof(std::pair<txn_id_t, lsn_t>));
    offset += att_size_ * sizeof(std::pair<txn_id_t, lsn_t>);
    // aborted_txns
    ...
    memcpy(aborted_txns_vec_.data(), src + offset,
           aborted_txns_size_ * sizeof(txn_id_t));
    offset += aborted_txns_size_ * sizeof(txn_id_t);
    // dpt
    ...
    memcpy(dpt_vec_.data(), src + offset,
           dpt_size_ * sizeof(std::pair<page_id_t, lsn_t>));
    offset += dpt_size_ * sizeof(std::pair<page_id_t, lsn_t>);
    ...
    memcpy(tab_name_offset_vec_.data(), src + offset,
           tab_name_offset_size_ * sizeof(size_t));
    offset += tab_name_offset_size_ * sizeof(size_t);
    // tab_name_str
