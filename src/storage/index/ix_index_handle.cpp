...
page_id_t IxNodeHandle::internal_lookup(const char *key) {
  ...
  page_id_t child_page_id = value_at(pos);
...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid, int n) {
  ...
  if (pos < 0 || pos > page_hdr->num_key) {
    throw InternalError("IxNodeHandle::insert_pairs Error: Invalid position");
  }
...
void IxNodeHandle::erase_pair(int pos) {
  ...
  if (pos < 0 || pos >= page_hdr->num_key) {
    throw InternalError("IxNodeHandle::erase_pair Error: Invalid position");
  }
...
std::pair<IxNodeHandle *, bool> IxIndexHandle::FindLeafPage(const char *key, Operation operation, bool find_first) {
  ...
  while (!current_node->is_leaf_page()) {
    ...
    page_id_t child_page_no = current_node->internal_lookup(key);

    // Unpin the current node before moving to the child
    buffer_pool_manager_->UnpinPage(current_node->get_page_id(), false);

    // Fetch the child node
    current_node = fetch_node(child_page_no);
  }
...
bool IxIndexHandle::GetValue(const char *key, std::vector<Rid> *result) {
  ...
  buffer_pool_manager_->UnpinPage(leaf_node->get_page_id(), false);
  delete leaf_node;
...
IxNodeHandle *IxIndexHandle::split(IxNodeHandle *node) {
  ...
  new_node->insert_pairs(0, node->get_key(split_point), node->get_rid(split_point), new_num);
  ...
  if (node->is_leaf_page()) {
    ...
    auto next_leaf = new_node->get_next_leaf();
    if (next_leaf != IX_NO_PAGE) {
      IxNodeHandle *next_leaf_node = fetch_node(next_leaf);
      next_leaf_node->set_prev_leaf(new_node->get_page_no());
      buffer_pool_manager_->UnpinPage(next_leaf_node->get_page_id(), true);
 ...
 * @note new_root 没有unpin, 且 old root(old_node) 会被调用者 unpin(fetch_node/find_leaf_page)
 */
void IxIndexHandle::insert_into_parent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node) {
  ...
  if (old_node->is_root_page()) {
    ...
    new_root->set_parent_page_no(IX_NO_PAGE);            // New root has no parent
    new_root->set_size(0);
    new_root->page_hdr->is_leaf = false;

    // Insert (old_node, new_node) into new root
    // Note: insert_pair will update num_key
    new_root->insert_pair(0, old_node->get_key(0), {old_node->get_page_no(), 0});
    new_root->insert_pair(1, new_node->get_key(0), {new_node->get_page_no(), 0});
    ...
    // // Unpin the new root
    // buffer_pool_manager_->UnpinPage(new_root->get_page_id(), true);
    delete new_root;
    return;
  }
  ...
  IxNodeHandle *parent_node = fetch_node(parent_page_no);

  // 3. Insert (key, new_node) into parent node
  Rid new_node_rid = {new_node->get_page_no(), 0};
  int insert_pos = parent_node->find_child(old_node) + 1;
  parent_node->insert_pair(insert_pos, key, new_node_rid);
  ...
  if (parent_node->get_size() >= parent_node->get_max_size()) {
    ...
    char *middle_key = new_sibling->get_key(0);
    insert_into_parent(parent_node, middle_key, new_sibling);
    // unpin for split: TOCHECK
    buffer_pool_manager_->UnpinPage(new_sibling->get_page_id(), true);
    delete new_sibling;
  }

  // Unpin the parent node
  buffer_pool_manager_->UnpinPage(parent_node->get_page_id(), true);
  delete parent_node;
}
...
page_id_t IxIndexHandle::InsertEntry(const char *key, const Rid &value) {
  ...
  if (new_size >= leaf_node->get_max_size()) {
    ...
    if (leaf_node->get_page_no() == file_hdr_->last_leaf_) {
      file_hdr_->last_leaf_ = new_sibling->get_page_no();
    }
    ...
    insert_into_parent(leaf_node, middle_key, new_sibling);
  ...
  buffer_pool_manager_->UnpinPage(leaf_node->get_page_id(), true);
...
bool IxIndexHandle::DeleteEntry(const char *key) {
  ...
  bool should_delete_node = coalesce_or_redistribute(leaf_node, &root_is_latched);

  // TODO: 4. Handle concurrent deletion and node removal if necessary
  if (should_delete_node) {
    release_node_handle(*leaf_node);
  }
 ...
 */
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, bool *root_is_latched) {
  // Todo:
  // 1. 判断node结点是否为根节点
  ...
  if (node->is_root_page()) {
    // 1.1 If it is the root, call adjust_root()
  ...
  IxNodeHandle *sibling_node = fetch_node(parent_node->value_at(sibling_index));
  ...
  if (node->get_size() + sibling_node->get_size() >= 2 * node->get_min_size()) {
    ...
    if (del_parent) {
      release_node_handle(*parent_node);
    }
    delete_node = true;
  }

  // Unpin the parent and sibling nodes that were pinned in 'fetch_node'
  buffer_pool_manager_->UnpinPage(parent_node->get_page_id(), true);
  buffer_pool_manager_->UnpinPage(sibling_node->get_page_id(), true);
  delete parent_node;
  delete sibling_node;
...
bool IxIndexHandle::adjust_root(IxNodeHandle *old_root_node) {
  ...
  if (!old_root_node->is_leaf_page() && old_root_node->get_size() == 1) {
    ...
    IxNodeHandle *new_root_node = fetch_node(new_root_page_no);
    ...
    buffer_pool_manager_->UnpinPage(new_root_node->get_page_id(), true);
...
void IxIndexHandle::redistribute(IxNodeHandle *neighbor_node, IxNodeHandle *node, IxNodeHandle *parent, int index) {
  ...
  if (index > 0) {
    ...
    int neighbor_last_index = neighbor_node->get_size() - 1;

    // Insert the last key-value pair from neighbor_node to the front of node
    node->insert_pairs(0, neighbor_node->get_key(neighbor_last_index), neighbor_node->get_rid(neighbor_last_index), 1);

    // Remove the last key-value pair from neighbor_node
    neighbor_node->erase_pair(neighbor_last_index);

    // Update the key in the parent node
    maintain_parent(node);
    // // This below line is not enough because of cascading updates
    // char *parent_key = node->get_key(0);
    // parent->set_key(index, parent_key);
    ...
    Rid neighbor_first_rid = *neighbor_node->get_rid(0);

    // Insert the first key-value pair from neighbor_node to the end of node
    node->insert_pairs(node->get_size(), neighbor_first_key, &neighbor_first_rid, 1);
    ...
    // // This below line is not enough because of cascading updates
    // char *parent_key = neighbor_node->get_key(0);
    // auto neighbor_parent = fetch_node(neighbor_node->get_parent_page_no());
    // neighbor_parent->set_key(neighbor_parent->find_child(neighbor_node), parent_key);
    // buffer_pool_manager_->UnpinPage(neighbor_parent->get_page_id(), true);
...
bool IxIndexHandle::coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
  ...
  int start_pos = (*neighbor_node)->get_size();
  int num_to_move = (*node)->get_size();
  (*neighbor_node)->insert_pairs(start_pos, (*node)->get_key(0), (*node)->get_rid(0), num_to_move);

  // If internal node, update children's parent pointers
  if (!(*node)->is_leaf_page()) {
    for (int i = 0; i < num_to_move; ++i) {
      maintain_child(*neighbor_node, start_pos + i);
    }
  } else {
  ...
  return coalesce_or_redistribute(*parent, root_is_latched);
}
...
Rid IxIndexHandle::get_rid(const Iid &iid) const {
  ...
  buffer_pool_manager_->UnpinPage(node->get_page_id(), false);  // unpin it!
...
Iid IxIndexHandle::lower_bound(const char *key) {
  ...
  if (leaf_node == nullptr) {
    throw InternalError("IxIndexHandle::lower_bound Error: Leaf node not found");
    // return Iid{-1, -1};
  }

  // 2. Use the lower_bound method in IxNodeHandle to find the appropriate key index within the leaf node
  int key_index = leaf_node->lower_bound(key);

  Iid result;
  // target key > all keys in leaf node
  if (key_index == leaf_node->get_size()) {
    if (leaf_node->get_page_no() == file_hdr_->last_leaf_) {
      // the last leaf node
      result = Iid{leaf_node->get_page_no(), leaf_node->get_size()};
    } else {
      // the leaf node has next leaf
      result = Iid{leaf_node->get_next_leaf(), 0};
    }
  } else {
    result = Iid{leaf_node->get_page_no(), key_index};
  }
...
Iid IxIndexHandle::upper_bound(const char *key) {
  ...
  if (leaf_node == nullptr) {
    throw InternalError("IxIndexHandle::upper_bound Error: Leaf node not found");
    // return Iid{-1, -1};
  }

  // 2. Use the upper_bound method in IxNodeHandle to find the appropriate key index within the leaf node
  int key_index = leaf_node->upper_bound(key);
...
void IxIndexHandle::maintain_parent(IxNodeHandle *node) {
  ...
  while (curr->get_parent_page_no() != IX_NO_PAGE) {
    ...
    if (memcmp(parent_key, child_first_key, file_hdr_->col_tot_len_) == 0) {
      assert(buffer_pool_manager_->UnpinPage(parent->get_page_id(), true));
      break;
    }
...
void IxIndexHandle::erase_leaf(IxNodeHandle *leaf) {
  ...
  buffer_pool_manager_->UnpinPage(prev->get_page_id(), true);

  IxNodeHandle *next = fetch_node(leaf->get_next_leaf());
  next->set_prev_leaf(leaf->get_prev_leaf());  // 注意此处是SetPrevLeaf()
  buffer_pool_manager_->UnpinPage(next->get_page_id(), true);
}
...
void IxIndexHandle::maintain_child(IxNodeHandle *node, int child_idx) {
  if (!node->is_leaf_page()) {
    ...
    buffer_pool_manager_->UnpinPage(child->get_page_id(), true);
  }
}
