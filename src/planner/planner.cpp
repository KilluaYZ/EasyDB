...
std::vector<Condition> pop_conds(std::vector<Condition> &conds, std::string tab_names) {
    ...
    auto it = conds.begin();
    while (it != conds.end()) {
        if ((tab_names.compare(it->lhs_col.tab_name) == 0 && it->is_rhs_stmt) || (tab_names.compare(it->lhs_col.tab_name) == 0 && it->is_rhs_val) || (it->lhs_col.tab_name.compare(it->rhs_col.tab_name) == 0)) {
        ...
        while (it != conds.end()) {
            ...
            if(enable_nestedloop_join && enable_sortmerge_join) {
                ...
                if (left_index_exist &&  right_index_exist) {  // join列存在索引
                    ...
                    left = std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, it->lhs_col.tab_name, noconds,
                                                      index_col_name_left);
                    right = std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, it->rhs_col.tab_name, noconds,
                                                       index_col_name_right);

                    table_join_executors = std::make_shared<JoinPlan>(T_IndexMerge, std::move(left), std::move(right), join_conds);
                } else {  // 不存在索引
...
std::shared_ptr<Plan> Planner::generate_aggregation_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan)
{
    ...
    return std::make_shared<AggregationPlan>(T_Aggregation, std::move(plan), query->cols, query->groupby_cols,query->having_conds);
}
