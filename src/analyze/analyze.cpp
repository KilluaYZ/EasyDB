...
std::shared_ptr<Query> Analyze::do_analyze(
  ...
  if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse)) {
    ...
    for (auto &sv_sel_col : x->cols) {
      ...
      // TabCol sel_col = {.tab_name = sv_sel_col->tab_name, .col_name =
      // sv_sel_col->col_name};
    ...
    // not select any specific column(select *), then select all columns in all
    // tables
    if (query->cols.empty()) {
      // select all columns
    ...
    // COUNT(*)
    //  else if (query->cols.size() == 1 && query->cols[0].col_name.empty() &&
    //  query->cols[0].aggregation_type == COUNT_AGG){
    ...
    if (x->has_having) {
      get_clause(x->having,
                 query->having_conds);  // maybe buggy (lack of robustness)
      check_clause(query->tables,
                   query->having_conds);  // fill table info for cols
    }
...
void Analyze::get_clause(
  ...
  for (auto &expr : sv_conds) {
    ...
    if (!expr->rhs) {
      ...
      } else {  // e.g. department_id IN (SELECT id FROM departments WHERE name
                // = 'HR');
        // IN operator, return a column
        cond.is_rhs_val = false;
        cond.is_rhs_stmt = true;
        // cautious: the comparison col is newly-generated, not exist in current
        // table
        cond.rhs_col = {.tab_name = rhs_stmt->tabs[0],
                        .col_name = rhs_stmt->cols[0]->col_name,
                        .aggregation_type = rhs_stmt->cols[0]->aggregation_type,
      ...
      // tab_name and col_name info of rhs_col are filled in next step
      // check_clause
      if (rhs_stmt) {
  ...
  for (auto &cond : conds) {
    ...
    if (cond.lhs_col.col_name.empty() &&
      ...
      if (!cond.is_rhs_val) {
        if (!cond.is_rhs_stmt) {
          cond.rhs_col = check_column(all_cols, cond.rhs_col);
        } else if (cond.is_rhs_exe_processed) {  // is processed IN subquery
                                                 // tuple
          continue;
        } else {  // rhs is subquery stmt, then table name should have been
                  // specified.
          auto temp_rhs_stmt =
...
ArithOp Analyze::convert_sv_arith_op(ast::SvArithOp op) {
  std::map<ast::SvArithOp, ArithOp> m = {{ast::SV_OP_PLUS, OP_PLUS},
